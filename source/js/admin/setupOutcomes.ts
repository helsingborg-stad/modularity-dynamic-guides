...
class SetupOutcomes {
	...
	public createOptions(select: HTMLSelectElement, detail: EventDetailObject) {
		...
		if ((globalState as GlobalState)[key]["heading"]) {
			...
			const option = optgroup?.querySelector(
	...
	private addOption(
		...
		optgroup: Element | undefined,
	) {
		if (option && optgroup) {
			option.setAttribute(
				"value",
				(globalState as GlobalState)[key][choiceKey],
			);
	...
	private addOptgroup(select: HTMLElement, key: string, optgroup: Element) {
		if (optgroup) {
			optgroup.setAttribute(
				"label",
				(globalState as GlobalState)[key]["heading"],
			);
		} else {
